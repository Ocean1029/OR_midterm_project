...
# Holding cost + (Purchasing cost + Variable shipping cost + Fixed shipping cost) + Container cost + Shortage cost
...
for i in S_I:
    for t in S_T:
        ...
        if t == 0:
            model.addConstr(v[i, t] == in_inventory + I_0[i] + I[i, t] - D[i, t] + l[i, t], name=f"InvBalance_{i}_{t}")
        else:
            model.addConstr(v[i, t] == v[i, t-1] + in_inventory + I[i, t] - D[i, t] + l[i, t], name=f"InvBalance_{i}_{t}")
            model.addConstr(v[i, t-1] >= D[i, t] - l[i, t], name=f"Demand_{i}_{t}")
...
# Relate lost sales and demand (7)
for i in S_I:
    for t in S_T:
        if t == 0:
